...
const getMockAiResponse = (name: string, category: string) => {
  let description = '';
  let tags: string[] = [];
...
export async function POST(request: Request) {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      await delay(1000);
      return NextResponse.json(getMockAiResponse(name, category));
    }
    ...
    try {
      const { name, category } = await request.clone().json();
      return NextResponse.json(getMockAiResponse(name, category));
    } catch {
      return NextResponse.json({ error: 'AI 상품 설명 생성에 실패했습니다.' }, { status: 500 });
    }
  }
}
